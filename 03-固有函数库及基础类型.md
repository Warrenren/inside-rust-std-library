...
```
下面的take及replace对于编程来说非常重要，`Option<T>`很多时候在结构体中需要指针时作为成员出现。如果这些结构被指针链接起来。那因为所有权及借用的关系，很多时候不能用赋值的办法对Option做更改，此时就需要使用take将Option<T>所有权转移出来，更改后，再用replace将Option<T>的所有权转移回去。   
```rust
...
```
`Option<T>`的take及replace组合可以完成对Option的不修改内存地址的内容修改。在Option被用于指针时，这个组合方法会很有市场，但因为引入两次拷贝，降低效率。所以当采用这种形式解决方案时，要考虑是否可以用unsafe的方式得到性能更高的方案。
