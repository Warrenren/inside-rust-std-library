...
目前可以多线程并发操作的类型有：
1. 内部可变性类型引用，
2. 具有递归到内部可变性类型成员的复合类型引用, 并且这些成员的引用可被模块外获得或通过方法做改变
3. 具有递归到内部可变性引用类型成员的复合类型及其引用，并且这些成员的可被获得或通过方法做
4. `*const T/* mut T/NonNull<T>/Unique<T>`及引用，
5. 具有递归到4类型成员的复合类型及其引用，并且这些成员可被模块外获得或通过方法做改变
6. 支持多份所有权的智能指针，
7. 能够递归到多份所有权智能指针类型成员的复合类型及其引用，并且这些成员可被模块外获得 
...
//Send trait对基本类型默认支持，如果复合类型中所有成员类型都支持Send，则此类型会默认自动支持Send。对于违反这一规则的复合类型，需要给出对Send trait显式的支持或不支持的实现。
//例如：
...
// 与Send trait类似，基本类型默认支持Sync。如果复合类型所有成员类型都支持Sync，则该类型会默认自动支持Sync，如果该复合类型实际不支持Sync，则需要显式给出Sync trait的实现定义。
//例如：
