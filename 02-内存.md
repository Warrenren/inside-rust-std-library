# RUST标准库内存相关模块代码分析
理解RUST程序的最关键点就是理解RUST内存相关的标准库代码。内存标准库代码很好的阐明了RUST的最基本的一些规则如所有权转移，借用，生命周期。
...
ptr模块主要包含原生指针`*const T/*mut T`, 原生指针封装`NonNull<T>/Unique<T>`, 本节主要集中在原生指针的类型结构及少量方法，为后继的mem模块分析做准备。
...
ptr模块的函数大部分逻辑都比较简单。很多就是对intrinsic 函数做调用。不做过多的分析。
...
```rust
    ...
    pub fn set_ptr_value(mut self, val: *const u8) -> Self {
        // 虽然转换很复杂，最终将self转换为一个瘦指针， self原先可能是一个胖指针，
...
RUST常用的这种封装类型。因为* const T是没有所有权，借用，生命周期这些属性的，完全脱离了RUST编译器的管理。但用NonNull<T>封装后，NonNull作为一个类型变量，具备了所有权，借用，生命周期这些属性, RUST编译器就可以管理了。
...
pub macro addr_of($place:expr) {
    ...
    //区别&于, &要求地址必须满足字节对齐和初始化，&raw 则没有这个问题
    &raw const $place
}
```
...
```rust
    ...
    //from_raw_in用Unique包装raw，使得raw的所有权被Unique获得
